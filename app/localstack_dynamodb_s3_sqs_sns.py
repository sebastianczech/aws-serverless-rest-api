...
if __name__ == "__main__":
    ...
    dynamodb_create_table(localstack_config_for_service('dynamodb', localstack_config()), 'localstack_dynamodb_python')
    ...
    dynamodb_delete_table(localstack_config_for_service('dynamodb', localstack_config()), 'localstack_dynamodb_python')

    # Create bucket if not exists
    s3_create_bucket(localstack_config_for_service('s3', localstack_config()), 'localstack-s3-python')

    # Upload binary data
    s3_upload_file(localstack_config_for_service('s3', localstack_config()), 'localstack-s3-python')
    ...
    print("\nFiles in localstack-s3-python:")
    for obj in s3_list_object_in_bucket(localstack_config_for_service('s3', localstack_config()), 'localstack-s3-python'):
        print("- " + obj['Key'])

    # Create SNS topic
    sns_create_topic(localstack_config_for_service('sns', localstack_config()), 'localstack_sns_python')
    ...
    # Create SQS queue
    sqs_create_queue(localstack_config_for_service('sqs', localstack_config()), 'localstack_sqs_python')
